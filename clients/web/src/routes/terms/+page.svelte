<script lang="ts">
	...
	let title = $state('Terms of Service');
	let body = $state('');
	...
	onMount(async () => {
		try {
			const doc = await api.get<{ title: string; body: string }>('/legal/terms');
			title = doc.title;
			body = doc.body;
		} catch (err) {
			error = err instanceof Error ? err.message : 'Failed to load terms of service';
		} finally {
			loading = false;
		}
	});
