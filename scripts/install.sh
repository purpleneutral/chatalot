...
# Detect ARM and offer pre-built images
USE_PREBUILT=""
ARCH=$(uname -m)
if [ "$ARCH" = "aarch64" ] || [ "$ARCH" = "arm64" ]; then
    echo ""
    echo -e "  ${YELLOW}ARM64 detected.${NC} Building from source takes ${BOLD}20+ minutes${NC} on ARM."
    echo -e "  Pre-built images are available and start in under a minute."
    echo ""
    if prompt_yes_no "Use pre-built image? (recommended for ARM)" "y"; then
        USE_PREBUILT="yes"
        # Create override to use pre-built image
        if [ ! -f "$OVERRIDE_FILE" ]; then
            cat > "$OVERRIDE_FILE" <<ARMEOF
# Auto-generated by install.sh — pre-built image for ARM64
services:
  chatalot:
...
ARMEOF
  ...
  # Pre-built image for ARM64 (added by install.sh)
  chatalot:
    image: ghcr.io/purpleneutral/chatalot:latest
ARMEOF
        fi
        print_success "Configured pre-built image from GHCR"
    fi
fi

echo ""
if [ -n "$USE_PREBUILT" ]; then
    echo -e "  This will pull the pre-built image and start the containers."
    echo -e "  First pull takes ${BOLD}1-2 minutes${NC} depending on your connection."
else
...
echo ""
if [ -n "$USE_PREBUILT" ]; then
    echo -e "  Pulling and starting containers..."
else
...
BUILD_FLAG=""
[ -z "$USE_PREBUILT" ] && BUILD_FLAG="--build"

if [ -n "$COMPOSE_PROFILE" ]; then
    docker compose --profile "$COMPOSE_PROFILE" up -d $BUILD_FLAG 2>&1 | while IFS= read -r line; do
        echo "  $line"
    done
else
    docker compose up -d $BUILD_FLAG 2>&1 | while IFS= read -r line; do
        echo "  $line"
    done
fi
